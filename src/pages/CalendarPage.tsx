...
export default function CalendarPage() {
  ...
  const { days, blanks } = useMemo(() => {
    ...
    return {
      ...
      blanks: Array.from({ length: firstDayOfMonth }, (_, i) => i),
    };
  }, [currentDate]);
            ...
            <div className="border-4 border-black dark:border-white bg-white dark:bg-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,1)]">
              ...
              <div className="grid grid-cols-7 border-b-4 border-black dark:border-white bg-neutral-100 dark:bg-neutral-900">
                {WEEKDAYS.map((day) => (
                  <div
                    key={day}
                    className="p-4 text-center font-extrabold uppercase tracking-widest border-r-4 last:border-r-0 border-black dark:border-white text-sm md:text-base"
                  >
              ...
              <div className="grid grid-cols-7 auto-rows-fr min-h-[600px]">
                {blanks.map((blank) => (
                  <div
                    key={`blank-${blank}`}
                    className="border-b-2 border-r-2 border-black dark:border-white bg-neutral-50 dark:bg-neutral-950/30 p-2 min-h-[120px]"
                  />
                ))}
                ...
                {days.map((day) => {
                  ...
                  const hasClash = dayTasks.some((t) =>
                    dashboard.isClashing(t.id),
                  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        <span
                          className={`text-lg font-extrabold w-8 h-8 flex items-center justify-center ${
                            isToday
